    ...
    public Car save(Car car) {
        if (car.getId() != null) {
            return repository.findById(car.getId())
                    .map(carToBeUpdated -> {
                        ...
                        if(!car.getLocation().getLat().equals(carToBeUpdated.getLocation().getLat())
                                || !car.getLocation().getLon().equals(carToBeUpdated.getLocation().getLon()))
            ...
            return Optional.of(car).map(carToBeSaved->{
                ...
                carWithId.setPrice(priceClient.getPrice(carWithId.getId()));
                return repository.save(carWithId);
            }).orElseThrow(CarNotFoundException::new);
